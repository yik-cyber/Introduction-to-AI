...
def randomTrainingExample(all_categories, category_lines, chars):   
    ...
    input_line_tensor  = torch.zeros(len(line), 1, num_char) #(line_len, char_num)
...
def ExampleForGeneration (all_categories, category, chars, letter='A'):
    ...
    category_tensor = torch.zeros(1, len(all_categories)) #1, n_category
    category_tensor[0][all_categories.index(category)] = 1 #one hot

    num_char = len(chars)+1

    input_line_tensor = tensor = torch.zeros( 1, num_char) #1, num_char
